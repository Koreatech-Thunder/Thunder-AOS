...
class ThunderBehaviorSpec : BehaviorSpec({
    ...
    given("사용자가 특정 번개를 참여중인 유저A 를 클릭했을 때") {
        ...
        `when`("서버 통신을 실패했다면") {
            then("`유저의 정보를 불러오는데 실패했습니다. 다시 시도해주세요.` 토스트를 띄운다.") {
            }
        }
    }
    ...
    given("사용자가 번개 참여하기 버튼을") {
        `when`("번개 제한 인원이") {
            ...
            and("인원 TO 가 있는 상태에서 참여하기 버튼을 누르면") {
                and("서버 통신을 성공한다면") {
                    then("참여 완료했습니다. 토스트를 띄우고, 적용된 번개 뷰를 보여준다.") {
                    }
                }
                and("서버 통신을 실패한다면") {
    ...
    given("사용자가 번개 취소하기 버튼을 눌렀을 때") {
        `when`("재확인 다이얼로그를 띄운 후에") {
            and("취소하기 버튼을 누르고") {
                and("서버 통신을 성공한다면") {
                    then("번개가 취소 되었습니다. 토스트를 띄우고, 적용된 번개 뷰를 보여준다.")
                }
                and("서버 통신을 실패한다면") {
